...
async function publishDirectory(dir) {
  if (!dir) {
    throw new Error(
      `must be invoked with an absolute or relative path to a directory.`
    );
  }
  ...
  const url =
    process.env.url ||
    `https://ipfs.infura.io:5001/api/v0/add?pin=${shouldPin}&cid-version=1&hash=${hashAlgo}`;
  ...
  for (const obj of ret) {
    if (obj.Name === "_") {
      ...
      console.info(
        `Example dns entry: _dnslink.example.tld. TXT "dnslink=/ipfs/${obj.Hash}"`
      );
      break;
    }
  }
